...
def generic_profile_fft_new(rhos, xs,cmaxs,rss,zs,ks,do_mass_norm=True):
    """
    Generic profile FFTing
    rhofunc_x: function that accepts vector spanning linspace(0,xmax,nxs)
    xmax:  some O(10-1000) dimensionless number specifying maximum of real space
...
def fft_integral_r(x,y,axis=-1):
    ...
    step = extent/N
    integrand = x*y
    uk = -np.fft.rfft(integrand,axis=axis).imag*step[..., None]
    ks = np.fft.rfftfreq(N, step[..., None]) *2*np.pi
    return ks,uk


def generic_profile_fft_r(rhofunc_x,xs,cmaxs,rss,zs,ks,do_mass_norm=True):
    """
    Generic profile FFTing
    rhofunc_x: function that accepts vector spanning linspace(0,xmax,nxs)
    xmax:  some O(10-1000) dimensionless number specifying maximum of real space
    ...
    # u(kt)
    integrand = rhos*theta
    kts,ukts = fft_integral_r(xs,integrand)
